...
Run `make benchmark-site` after changing either source so the homepage and this document stay aligned. The homepage stays latency-first, and this notes page carries the issue #46 methodology decision about when, where, and how quality metrics may be published.

## Accuracy Publishing Policy

Issue #46 resolves to a simple public-facing rule:

- The homepage comparison and docs index stay latency-only. Public benchmark tables on those landing surfaces should not show WER today.
...
- When official accuracy coverage lands, keep it on a separate methodology/details surface first instead of mixing quality claims into the primary latency matrix.
- Upstream model-card WER is useful as background research, but it is not a substitute for checked-in repo evaluation artifacts and should not be rendered as if it were a measurement of this serving stack.
...
2. Keep the homepage latency-only and link to this methodology page for any future quality claims.
...
These rows match the current manifest entries used on the homepage, plus two doc-only Parakeet MLX CLI preview artifacts. Every distinct runtime setup keeps its own row here, even when multiple lanes share the same underlying model. The latency and throughput fields come from checked-in repo artifacts. No WER column is shown in this comparison by design: issue #46 recommends keeping the public comparison latency-only until the repo publishes a separate annotated quality track. The new `parakeet-mlx-service-110m` row is the warmed service-style counterpart to the earlier cold CLI preview.

| Track | Samples | REST Mean / P95 | REST RTF | Partial Mean / P95 | Final Mean / P95 | Artifact |
| --- | ---: | --- | ---: | --- | --- | --- |
| `parakeet-mlx-service-110m` | 10 | 141.9 ms / 170.0 ms | 0.020 | 74.5 ms / 96.9 ms | 210.6 ms / 246.3 ms | `docs/benchmark-results/parakeet-mlx-110m-service-2026-06-13.json` |
| `parakeet-nemo-compose` | 10 | 331.4 ms / 511.5 ms | 0.046 | 148.5 ms / 245.8 ms | 379.0 ms / 633.5 ms | `docs/benchmark-results/parakeet-nemo-110m-compose-2026-06-09.json` |
| `faster-whisper-base-c80-w075-json-preview` | 1 | 537.4 ms / 537.4 ms | 0.074 | 15.4 ms / 15.4 ms | 46867.0 ms / 46867.0 ms | `docs/benchmark-results/faster-whisper-base.en-int8-c80-w0_75-json-2026-06-10.json` |
| `faster-whisper-base` | 10 | 573.3 ms / 741.1 ms | 0.079 | 553.0 ms / 2451.5 ms | 560.2 ms / 761.2 ms | `docs/benchmark-results/faster-whisper-base.en-int8-2026-06-10.json` |
| `qwen-mps` | 10 | 1186.2 ms / 1261.2 ms | 0.163 | 352.0 ms / 445.5 ms | 1189.6 ms / 1248.2 ms | `docs/benchmark-results/qwen-mps-2026-06-10.json` |
| `faster-whisper-small` | 10 | 1378.3 ms / 1531.1 ms | 0.189 | 1023.2 ms / 1202.4 ms | 1420.6 ms / 1514.0 ms | `docs/benchmark-results/faster-whisper-small.en-int8-2026-06-10.json` |
| `parakeet-mlx-110m` | 3 | 1360.7 ms / 1716.2 ms | n/a (CLI artifact) | n/a (CLI artifact) | n/a (CLI artifact) | `docs/benchmark-results/parakeet-mlx-110m-2026-06-13.json` |
| `parakeet-mlx` | 3 | 1971.9 ms / 2595.8 ms | n/a (CLI artifact) | n/a (CLI artifact) | n/a (CLI artifact) | `docs/benchmark-results/parakeet-mlx-2026-06-13.json` |
| `parakeet-compose` | 10 | 2388.3 ms / 4098.1 ms | 0.328 | 1715.1 ms / 2968.7 ms | 2215.8 ms / 3080.4 ms | `docs/benchmark-results/parakeet-compose-2026-06-10.json` |
| `qwen-compose` | 1 | 5482.2 ms / 5904.4 ms | 0.753 | 3696.1 ms / 6314.4 ms | 0.9 ms / 0.9 ms | `docs/benchmark-results/qwen-compose-2026-06-08.json` |

Notes:

- No WER is shown in the public comparison tables because upstream model-card numbers do not capture this repo's runtime choices such as CPU vs MPS, chunk/window cadence, websocket framing, warmup state, or transport overhead.
