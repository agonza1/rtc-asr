...
- `qwen-compose`: legacy 5-sample Compose CPU artifact refreshed on `2026-06-19` using `float16`, `3` REST runs per sample, and an `8`-chunk partial cadence; it remains published as supporting context until a newer `/v1/stt/stream` replacement is checked in.
