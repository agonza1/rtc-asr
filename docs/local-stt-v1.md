...
Raw UDS framing is tracked separately as an experimental issue 88 transport. The tested frame shape is a 5 byte little-endian header (`uint8 frame_type`, `uint32 payload_len_le`) followed by JSON control, PCM16 audio, JSON event, error, ping, or pong payload bytes. Ping and pong frames may carry either an empty payload or a JSON object with `ping_id` / `timestamp_ms`; clients may answer server keepalives with pong frames, and the service treats those acknowledgements as no-op control frames. The `/health` and `/api/models` protocol catalog exposes this as `raw_uds` with `status: codec_only` by default, plus `enabled`, `enable_env`, `path_env`, `path_env_fallbacks`, `plugin_config: {"transport": "raw_uds", "uds_path": "<socket>"}`, frame type codes, frame overhead, frame direction, keepalive payload shapes, lifecycle fields, active stream limits such as `limits.max_buffer_bytes`, low-power benchmark environment fields, optional externally observed power/thermal fields, and protocol-error coverage for malformed JSON, oversized payloads, incomplete frames, frame-length mismatches, server-only frame types sent by clients, and client-only frame types sent by servers so benchmark harnesses can discover the exact opt-in knobs and copy target-contract metadata into artifacts. Set `LOCAL_STT_RAW_UDS_ENABLED=true` and `LOCAL_STT_RAW_UDS_PATH=/run/rtc-asr/stt.raw.sock` to serve the raw UDS listener as an opt-in benchmark path; raw client config may also fall back to `LOCAL_STT_UDS_PATH` when `LOCAL_STT_RAW_UDS_PATH` is unset. Startup removes stale socket files and fails clearly if the path exists but is not a socket. Keep raw UDS experimental unless comparison artifacts show a p95 first-interim win of at least 5 ms over UDS WebSocket.
...
Local STT v1 keeps the same client message shapes for both decoder strategies:
...
Clients can inspect `/api/models` `local_stt_decoder` before connecting. Each `ready` and `transcript` event also repeats `metadata.decoder_mode` for the selected utterance. Backends that do not expose `start_stream` use `rolling_window`; backends that do expose it default new Local STT v1 sessions to `stateful` and keep `rolling_window` documented as the fallback mode. `ASR_BACKEND=vosk` is the first native stateful backend integration: `/api/models` reports `native_streaming.recognizer=vosk.KaldiRecognizer`, `state_scope=per_utterance`, and `incremental_input=new_pcm16_bytes_only`; at runtime it creates one recognizer per utterance, pushes only new PCM16 bytes into that recognizer, and releases the recognizer on finalize, cancel, close, disconnect, or error. The other bundled model adapters remain rolling-window compatibility backends until they expose a real per-utterance decoder API. `scripts/bench_local_stt_stream.py` records the observed `decoder_modes`, `decoder_mode_counts`, backend, model, and `decoder_compute_rtf` in each sample so before/after artifacts can prove which decoder path produced the measured compute cost.
