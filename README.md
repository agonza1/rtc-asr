...
Compose tags the default ASR image as `realtime-asr:faster-whisper-cpu`. The Dockerfile also exposes backend-specific build targets: `asr-faster-whisper-cpu`, `asr-qwen-cpu`, `asr-parakeet-transformers-cpu`, `asr-parakeet-nemo-cpu`, and `browser-pipecat-demo`. The backend benchmark targets use separate tags such as `realtime-asr:qwen-cpu`, `realtime-asr:parakeet-transformers-cpu`, and `realtime-asr:parakeet-nemo-cpu` so repeated builds can be inspected and compared without overwriting the default image.
...
The Compose image size report accepts readable image tag aliases such as `--tag`, `--tags`, and `--image-tags`, readable summary aliases such as `--summary`, `--summary-json`, `--summary-csv`, and `--summary-markdown`, readable budget aliases such as `--size-budget`, `--total-budget`, `--deduplicated-total-budget`, `--age-budget`, `--older-than`, and `--missing-budget`, plus readable sort aliases such as `--sort largest-to-smallest`, `--sort newest-to-oldest`, `--sort stale-first`, `--sort present-first`, `--sort missing-first`, `--sort known-size-first`, and `--sort known-created-first`, and readable gate aliases including `--all-images-present`, `--any-image-present`, `--require-known-image-size`, `--require-known-image-created`, `--known-image-id`, and `--unique-image-ids`. Size budgets accept units such as `MB` or `GiB`; image age budgets accept bare days plus `h`, `d`, and `w` suffixes. Summary output also reports deduplicated present-image size and shared-image size savings so tag aliases do not hide the actual local Docker footprint. Use `make compose-image-hygiene-check` for a review-friendly 14-day metadata and duplicate-ID gate before pruning local Compose images.
