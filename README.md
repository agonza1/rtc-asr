...
The Compose image size report accepts readable image tag aliases such as `--tag`, `--tags`, and `--image-tags`, readable summary aliases such as `--summary`, `--summary-json`, `--summary-csv`, and `--summary-markdown`, readable budget aliases such as `--size-budget`, `--total-budget`, `--deduplicated-total-budget`, and `--age-budget`, plus readable sort aliases such as `--sort largest-to-smallest`, `--sort newest-to-oldest`, `--sort stale-first`, `--sort present-first`, `--sort missing-first`, `--sort known-size-first`, and `--sort known-created-first`, and readable gate aliases including `--all-images-present`, `--any-image-present`, `--require-known-image-size`, `--require-known-image-created`, and `--unique-image-ids`. Summary output also reports deduplicated present-image size and shared-image size savings so tag aliases do not hide the actual local Docker footprint.
