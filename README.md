...
The streaming backend artifact comparison script accepts path aliases such as `--baseline-artifact`, `--baseline-json`, `--candidate-artifact`, `--candidate-json`, `--json-output`, `--comparison-output`, `--markdown-report`, `--md-report`, `--summary-markdown`, and `--report-output`.
