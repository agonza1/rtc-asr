...
RUN /opt/venv/bin/pip install --upgrade pip && \
    if [ -n "$ENABLE_PIPECAT_DEMO_RUNTIME" ]; then \
      ...
      grep -Ev '^(faster-whisper|torch|qwen-asr|transformers==|accelerate|pytest|httpx)' requirements.txt > requirements.docker.txt && \
      /opt/venv/bin/pip install -r requirements.docker.txt && \
      if [ -n "$ENABLE_QWEN_RUNTIME" ]; then /opt/venv/bin/pip install --index-url https://download.pytorch.org/whl/cpu torch && /opt/venv/bin/pip install 'qwen-asr>=0.0.6' 'transformers==4.57.6' 'accelerate>=1.8.1'; fi && \
      if [ -n "$ENABLE_PARAKEET_RUNTIME" ]; then /opt/venv/bin/pip install --index-url https://download.pytorch.org/whl/cpu torch && /opt/venv/bin/pip install --upgrade --no-deps huggingface-hub==1.18.0 transformers==5.10.2; fi && \
      if [ -z "$ENABLE_QWEN_RUNTIME" ] && [ -z "$ENABLE_PARAKEET_RUNTIME" ] && [ -z "$ENABLE_NEMO_RUNTIME" ]; then /opt/venv/bin/pip install 'faster-whisper>=1.1.0'; fi && \
