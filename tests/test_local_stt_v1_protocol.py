...
@pytest.mark.parametrize("payload", [None, "not-object", ["ping"]])
def test_raw_uds_client_encoder_rejects_non_object_payloads(payload: object) -> None:
    with pytest.raises(LocalSttProtocolError) as excinfo:
...
def test_raw_uds_client_encoder_accepts_canonical_flat_start_protocol() -> None:
    encoded = encode_raw_uds_client_message(
        {
            "type": "start",
            ...
            "format": HOT_PATH_PCM_FORMAT,
            "frame_ms": HOT_PATH_FRAME_MS,
            "client_stream_id": "canonical-protocol",
        }
    )
    frame = decode_raw_uds_frame(encoded)
    ...
    assert start.type == "start"
    assert start.version == PROTOCOL_VERSION
...
def test_raw_uds_server_event_frame_classifies_malformed_json() -> None:
    frame = decode_raw_uds_frame(encode_raw_uds_frame(RawUdsFrameType.JSON_EVENT, b"{"))
...
def test_raw_uds_server_event_frame_classifies_schema_errors() -> None:
    frame = decode_raw_uds_frame(
        encode_raw_uds_json_frame(RawUdsFrameType.JSON_EVENT, {"type": "transcript"})
    )
