...
def test_normalized_transport_accepts_readable_target_transport_aliases() -> None:
    ...
    assert compare_module.normalized_transport({"target": {"transport": "tcp secure websocket transport"}}) == "tcp_ws"
    ...
    assert compare_module.normalized_transport({"target": {"transport": "unix domain socket transport"}}) == "uds_ws"
    assert compare_module.normalized_transport({"target": {"transport": "domain socket websocket"}}) == "uds_ws"
    assert compare_module.normalized_transport({"target": {"transport": "domain socket websocket transport"}}) == "uds_ws"
    assert compare_module.normalized_transport({"target": {"transport": "raw-unix-socket"}}) == "raw_uds"
    assert compare_module.normalized_transport({"target": {"transport": "raw uds transport"}}) == "raw_uds"
    assert compare_module.normalized_transport({"target": {"transport": "raw unix domain"}}) == "raw_uds"
    assert compare_module.normalized_transport({"target": {"transport": "raw unix domain socket"}}) == "raw_uds"
    assert compare_module.normalized_transport({"target": {"transport": "raw unix domain socket transport"}}) == "raw_uds"
...
def test_normalized_transport_accepts_punctuated_readable_aliases() -> None:
    ...
    assert compare_module.normalized_transport({"target": {"transport": "uds+websocket"}}) == "uds_ws"
    assert compare_module.normalized_transport({"target": {"transport": "websocket+uds"}}) == "uds_ws"
    assert compare_module.normalized_transport({"target": {"transport": "websocket unix socket"}}) == "uds_ws"
    assert compare_module.normalized_transport({"target": {"transport": "ws unix"}}) == "uds_ws"
...
def test_compare_artifacts_infers_run_count_from_sample_mapping(tmp_path: Path) -> None:
    tcp = write_artifact(tmp_path / "tcp.json", "tcp_ws", 18.0, runs=None)
    uds = write_artifact(tmp_path / "uds.json", "uds_ws", 17.0, runs=None)
    raw = write_artifact(tmp_path / "raw.json", "raw_uds", 11.0, runs=None)
    ...
    for path, count in ((tcp, 3), (uds, 4), (raw, 5)):
        payload = json.loads(path.read_text(encoding="utf8"))
        payload["samples"] = {f"run_{index}": {"index": index} for index in range(1, count + 1)}
        path.write_text(json.dumps(payload), encoding="utf8")
...
def test_compare_artifacts_accepts_string_and_plural_run_count_aliases(tmp_path: Path) -> None:
    tcp = write_artifact(tmp_path / "tcp.json", "tcp_ws", 18.0)
    uds = write_artifact(tmp_path / "uds.json", "uds_ws", 17.0)
    raw = write_artifact(tmp_path / "raw.json", "raw_uds", 11.0)
...
def test_compare_artifacts_accepts_send_queue_depth_ms_alias(tmp_path: Path) -> None:
    ...
    payload = json.loads(raw.read_text(encoding="utf8"))
    payload["summary"]["send_queue_depth_p95_ms"] = payload["summary"].pop("audio_send_queue_depth_p95_ms")
    raw.write_text(json.dumps(payload), encoding="utf8")
...
def test_compare_artifacts_blocks_mapped_sample_protocol_error_codes_without_diagnostics(tmp_path: Path) -> None:
    tcp = write_artifact(tmp_path / "tcp.json", "tcp_ws", 18.0)
    uds = write_artifact(tmp_path / "uds.json", "uds_ws", 17.0)
    raw = write_artifact(tmp_path / "raw.json", "raw_uds", 11.0)
    ...
    raw_payload["summary"].pop("protocol_errors")
    raw_payload["samples"] = {
        "run_1": {"protocol_error_codes": ["raw_uds_malformed_json_control"]},
        "run_2": {
...
def test_compare_artifacts_blocks_nonzero_diagnostic_protocol_error_total(tmp_path: Path) -> None:
    ...
    raw_payload = json.loads(raw.read_text(encoding="utf8"))
    raw_payload["summary"].pop("protocol_errors")
    raw_payload["diagnostics"] = {"protocol_error_total": 2}
    raw.write_text(json.dumps(raw_payload), encoding="utf8")
    ...
    assert comparison["blocking_gaps"] == [
        "raw_uds diagnostic protocol_error_total must be zero; got total=2 (no per-code counts)"
    ]


def test_compare_artifacts_reads_diagnostic_warning_total_aliases(tmp_path: Path) -> None:
    tcp = write_artifact(tmp_path / "tcp.json", "tcp_ws", 18.0)
    uds = write_artifact(tmp_path / "uds.json", "uds_ws", 17.0)
    raw = write_artifact(tmp_path / "raw.json", "raw_uds", 11.0)

    raw_payload = json.loads(raw.read_text(encoding="utf8"))
    raw_payload["diagnostics"] = {"warnings_total": "3"}
    raw.write_text(json.dumps(raw_payload), encoding="utf8")
...
def test_compare_artifacts_accepts_string_cpu_utilization_percent(tmp_path: Path) -> None:
    tcp = write_artifact(tmp_path / "tcp.json", "tcp_ws", 18.0, cpu_utilization_percent=None)
    uds = write_artifact(tmp_path / "uds.json", "uds_ws", 17.0, cpu_utilization_percent=None)
    raw = write_artifact(tmp_path / "raw.json", "raw_uds", 11.0, cpu_utilization_percent=None)

    for path, value in ((tcp, "14.5%"), (uds, " 12.25 % "), (raw, "10")):
        payload = json.loads(path.read_text(encoding="utf8"))
        payload["metrics"] = {"cpu": {"average_percent": value}}
        path.write_text(json.dumps(payload), encoding="utf8")
    ...
    assert comparison["transports"]["tcp_ws"]["cpu_utilization_percent"] == 14.5
    assert comparison["transports"]["uds_ws"]["cpu_utilization_percent"] == 12.25
    assert comparison["transports"]["raw_uds"]["cpu_utilization_percent"] == 10.0


def test_compare_artifacts_accepts_cpu_usage_aliases(tmp_path: Path) -> None:
    tcp = write_artifact(tmp_path / "tcp.json", "tcp_ws", 18.0, cpu_utilization_percent=None)
    uds = write_artifact(tmp_path / "uds.json", "uds_ws", 17.0, cpu_utilization_percent=None)
    raw = write_artifact(tmp_path / "raw.json", "raw_uds", 11.0, cpu_utilization_percent=None)

    for path, value in ((tcp, 18.0), (uds, "15.5%"), (raw, {"p95": 9.5})):
        payload = json.loads(path.read_text(encoding="utf8"))
        payload["metrics"] = {"cpu": {"usage_percent": value}}
        path.write_text(json.dumps(payload), encoding="utf8")
    ...
    assert comparison["cpu_utilization_coverage"]["complete"] is True
    assert comparison["lowest_cpu_utilization_percent_transport"] == "raw_uds"
    assert comparison["transports"]["tcp_ws"]["cpu_utilization_percent"] == 18.0
    assert comparison["transports"]["uds_ws"]["cpu_utilization_percent"] == 15.5
    assert comparison["transports"]["raw_uds"]["cpu_utilization_percent"] == 9.5


def test_compare_artifacts_accepts_top_level_cpu_usage_aliases(tmp_path: Path) -> None:
    tcp = write_artifact(tmp_path / "tcp.json", "tcp_ws", 18.0, cpu_utilization_percent=None)
    uds = write_artifact(tmp_path / "uds.json", "uds_ws", 17.0, cpu_utilization_percent=None)
    raw = write_artifact(tmp_path / "raw.json", "raw_uds", 11.0, cpu_utilization_percent=None)
    ...
    for path, value in ((tcp, 18.0), (uds, "15.5%"), (raw, {"p95": 9.5})):
        payload = json.loads(path.read_text(encoding="utf8"))
        payload["cpu"] = {"usage_percent": value}
        path.write_text(json.dumps(payload), encoding="utf8")
...
def test_compare_artifacts_requires_raw_uds_frame_direction_coverage(tmp_path: Path) -> None:
    tcp = write_artifact(tmp_path / "tcp.json", "tcp_ws", 18.0)
    uds = write_artifact(tmp_path / "uds.json", "uds_ws", 18.0)
    raw = write_artifact(tmp_path / "raw.json", "raw_uds", 12.0)
    raw_payload = json.loads(raw.read_text(encoding="utf8"))
    raw_payload["target"]["frame_direction"] = {
        "client_to_server": ["JSON_CONTROL", "AUDIO_PCM16"],
        "server_to_client": ["JSON_EVENT", "PING", "PONG"],
    }
    raw.write_text(json.dumps(raw_payload), encoding="utf8")
    ...
    assert comparison["raw_uds_frame_contract_gaps"] == [
        "raw_uds missing client_to_server frame direction coverage: PING,PONG",
        "raw_uds missing server_to_client frame direction coverage: ERROR",
    ]
    assert comparison["blocking_gaps"] == comparison["raw_uds_frame_contract_gaps"]
    assert comparison["raw_uds_recommendation_gate"]["blockers"] == [
        "frame_contract:raw_uds missing client_to_server frame direction coverage: PING,PONG",
        "frame_contract:raw_uds missing server_to_client frame direction coverage: ERROR",
    ]
...
def test_compare_artifacts_accepts_raw_uds_frame_direction_from_benchmark_contract(tmp_path: Path) -> None:
    ...
    raw = write_artifact(tmp_path / "raw.json", "raw_uds", 12.0)
    raw_payload = json.loads(raw.read_text(encoding="utf8"))
    raw_payload["target"].pop("frame_direction")
    raw_payload["target_contract"] = {
        "frame_direction": {
            "client_to_server": ["JSON_CONTROL", "AUDIO_PCM16", "PING", "PONG"],
            "server_to_client": ["JSON_EVENT", "ERROR", "PING", "PONG"],
    ...
    assert comparison["raw_uds_frame_contract_gaps"] == []
    assert comparison["transports"]["raw_uds"]["frame_direction"] == {
        "client_to_server": ["JSON_CONTROL", "AUDIO_PCM16", "PING", "PONG"],
        "server_to_client": ["JSON_EVENT", "ERROR", "PING", "PONG"],
    }
...
def test_compare_artifacts_accepts_health_catalog_frame_type_code_mapping(tmp_path: Path) -> None:
    tcp = write_artifact(tmp_path / "tcp.json", "tcp_ws", 18.0)
    uds = write_artifact(tmp_path / "uds.json", "uds_ws", 18.0)
    raw = write_artifact(tmp_path / "raw.json", "raw_uds", 12.0)
    raw_payload = json.loads(raw.read_text(encoding="utf8"))
...
def test_compare_artifacts_rejects_raw_uds_lifecycle_and_semantic_lifecycle_drift(tmp_path: Path) -> None:
    tcp = write_artifact(tmp_path / "tcp.json", "tcp_ws", 18.0)
    uds = write_artifact(tmp_path / "uds.json", "uds_ws", 18.0)
    raw = write_artifact(tmp_path / "raw.json", "raw_uds", 12.0)
    raw_payload = json.loads(raw.read_text(encoding="utf8"))
    raw_payload["target"]["semantic_lifecycle"] = ["start", "audio", "finalize", "transcript", "cancel", "close"]
    raw.write_text(json.dumps(raw_payload), encoding="utf8")

    comparison = compare_module.compare_artifacts([tcp, uds, raw])

    assert comparison["raw_uds_lifecycle_gaps"] == [
        "raw_uds semantic_lifecycle mismatch: expected start,audio,transcript,finalize,cancel,close; "
        "got start,audio,finalize,transcript,cancel,close"
    ]
    assert comparison["blocking_gaps"] == comparison["raw_uds_lifecycle_gaps"]
    assert comparison["raw_uds_recommendation_gate"]["blockers"] == [
        "lifecycle:raw_uds semantic_lifecycle mismatch: expected start,audio,transcript,finalize,cancel,close; "
        "got start,audio,finalize,transcript,cancel,close"
    ]
...
def test_compare_artifacts_requires_raw_uds_error_handling_coverage(tmp_path: Path) -> None:
    ...
    assert comparison["raw_uds_error_handling_gaps"] == [
        "raw_uds missing protocol-error handling coverage: malformed_json_control,invalid_json_payload,oversized_payload,incomplete_frame,frame_length_mismatch,invalid_client_frame_type,invalid_server_frame_type"
    ]
    assert comparison["blocking_gaps"] == comparison["raw_uds_error_handling_gaps"]
    assert comparison["raw_uds_recommendation_gate"]["blockers"] == [
        "error_handling:raw_uds missing protocol-error handling coverage: malformed_json_control,invalid_json_payload,oversized_payload,incomplete_frame,frame_length_mismatch,invalid_client_frame_type,invalid_server_frame_type"
    ]
    assert comparison["recommendation"] == (
...
def test_compare_artifacts_requires_raw_uds_error_code_coverage(tmp_path: Path) -> None:
    tcp = write_artifact(tmp_path / "tcp.json", "tcp_ws", 18.0)
    uds = write_artifact(tmp_path / "uds.json", "uds_ws", 18.0)
    raw = write_artifact(tmp_path / "raw.json", "raw_uds", 12.0)
    raw_payload = json.loads(raw.read_text(encoding="utf8"))
    raw_payload["target"]["error_codes"] = ["raw_uds_unsupported_frame_type"]
    raw.write_text(json.dumps(raw_payload), encoding="utf8")

    comparison = compare_module.compare_artifacts([tcp, uds, raw])

    assert comparison["raw_uds_error_code_gaps"] == [
        "raw_uds missing protocol-error code coverage: raw_uds_malformed_json_control,raw_uds_invalid_json,raw_uds_payload_too_large,raw_uds_incomplete_frame,raw_uds_frame_length_mismatch,raw_uds_invalid_client_frame_type,raw_uds_invalid_server_frame_type"
    ]
    assert comparison["blocking_gaps"] == comparison["raw_uds_error_code_gaps"]
    assert comparison["raw_uds_recommendation_gate"]["blockers"] == [
        "error_codes:raw_uds missing protocol-error code coverage: raw_uds_malformed_json_control,raw_uds_invalid_json,raw_uds_payload_too_large,raw_uds_incomplete_frame,raw_uds_frame_length_mismatch,raw_uds_invalid_client_frame_type,raw_uds_invalid_server_frame_type"
    ]
    assert comparison["recommendation"] == (
        "Re-run raw UDS benchmarks with protocol-error code coverage before recommending raw UDS."
    )
    assert comparison["raw_uds_decision_summary"]["next_action"] == (
        "Re-run raw UDS with protocol-error code coverage."
    )
    assert compare_module.comparison_has_blocking_gaps(comparison) is True


def test_compare_artifacts_accepts_raw_uds_error_codes_from_benchmark_contract(tmp_path: Path) -> None:
    tcp = write_artifact(tmp_path / "tcp.json", "tcp_ws", 18.0)
    uds = write_artifact(tmp_path / "uds.json", "uds_ws", 18.0)
    raw = write_artifact(tmp_path / "raw.json", "raw_uds", 12.0)
    raw_payload = json.loads(raw.read_text(encoding="utf8"))
    error_codes = raw_payload["target"].pop("error_codes")
    raw_payload["target_contract"] = {"error_codes": error_codes}
    raw.write_text(json.dumps(raw_payload), encoding="utf8")

    comparison = compare_module.compare_artifacts([tcp, uds, raw])

    assert comparison["raw_uds_error_code_gaps"] == []
    assert comparison["transports"]["raw_uds"]["error_codes"] == error_codes
...
def test_compare_artifacts_reads_process_cpu_utilization_aliases(tmp_path: Path) -> None:
    tcp = write_artifact(tmp_path / "tcp.json", "tcp_ws", 18.0, cpu_utilization_percent=None)
    uds = write_artifact(tmp_path / "uds.json", "uds_ws", 16.0, cpu_utilization_percent=None)
    raw = write_artifact(tmp_path / "raw.json", "raw_uds", 12.0, cpu_utilization_percent=None)

    tcp_payload = json.loads(tcp.read_text(encoding="utf8"))
    tcp_payload["environment"] = {"process_cpu_percent": 10.5}
    tcp.write_text(json.dumps(tcp_payload), encoding="utf8")
    uds_payload = json.loads(uds.read_text(encoding="utf8"))
    uds_payload["metrics"] = {"process": {"cpu_percent": 12.0}}
    uds.write_text(json.dumps(uds_payload), encoding="utf8")
    raw_payload = json.loads(raw.read_text(encoding="utf8"))
    raw_payload["cpu"] = {"utilization": 9.5}
    raw.write_text(json.dumps(raw_payload), encoding="utf8")
...
def test_compare_artifacts_reads_summary_cpu_utilization_percentiles(tmp_path: Path) -> None:
    tcp = write_artifact(tmp_path / "tcp.json", "tcp_ws", 18.0, cpu_utilization_percent=None)
    uds = write_artifact(tmp_path / "uds.json", "uds_ws", 16.0, cpu_utilization_percent=None)
    raw = write_artifact(tmp_path / "raw.json", "raw_uds", 12.0, cpu_utilization_percent=None)

    tcp_payload = json.loads(tcp.read_text(encoding="utf8"))
    tcp_payload["summary"]["cpu_utilization_percent"] = {"p50": 8.0, "p95": 10.5}
    tcp.write_text(json.dumps(tcp_payload), encoding="utf8")
    uds_payload = json.loads(uds.read_text(encoding="utf8"))
    uds_payload["summary"]["cpu"] = {"average_percent": {"p95": 12.0}}
    uds.write_text(json.dumps(uds_payload), encoding="utf8")
    raw_payload = json.loads(raw.read_text(encoding="utf8"))
    raw_payload["summary"]["process"] = {"cpu_percent": {"p50": 7.0, "p95": 9.5}}
    raw.write_text(json.dumps(raw_payload), encoding="utf8")
...
def test_compare_artifacts_reads_sample_cpu_utilization_evidence(tmp_path: Path) -> None:
    tcp = write_artifact(tmp_path / "tcp.json", "tcp_ws", 18.0, cpu_utilization_percent=None)
    uds = write_artifact(tmp_path / "uds.json", "uds_ws", 16.0, cpu_utilization_percent=None)
    raw = write_artifact(tmp_path / "raw.json", "raw_uds", 12.0, cpu_utilization_percent=None)

    for path, samples in (
        (tcp, [{"metrics": {"cpu": {"percent": 10.5}}}, {"metrics": {"cpu": {"percent": 11.0}}}]),
        (uds, [{"cpu_utilization_percent": "12.25%"}, {"cpu_utilization_percent": "12.0%"}]),
        (raw, {"run_1": {"process": {"cpu_percent": 9.0}}, "run_2": {"process": {"cpu_percent": 9.5}}}),
    ):
        payload = json.loads(path.read_text(encoding="utf8"))
    ...
    assert comparison["cpu_utilization_coverage"]["complete"] is True
    assert comparison["lowest_cpu_utilization_percent_transport"] == "raw_uds"
    assert comparison["transports"]["tcp_ws"]["cpu_utilization_percent"] == 11.0
    assert comparison["transports"]["uds_ws"]["cpu_utilization_percent"] == 12.25
    assert comparison["transports"]["raw_uds"]["cpu_utilization_percent"] == 9.5
...
def test_compare_artifacts_accepts_duration_seconds_benchmark_alias(tmp_path: Path) -> None:
    tcp = write_artifact(tmp_path / "tcp.json", "tcp_ws", 18.0)
    uds = write_artifact(tmp_path / "uds.json", "uds_ws", 18.0)
    raw = write_artifact(tmp_path / "raw.json", "raw_uds", 13.0)

    for path, duration_key in ((tcp, "duration_seconds"), (uds, "audio_duration_seconds"), (raw, "duration_s")):
        payload = json.loads(path.read_text(encoding="utf8"))
        payload["audio"].pop("duration_ms")
        payload["audio"][duration_key] = 1.0
        path.write_text(json.dumps(payload), encoding="utf8")

    comparison = compare_module.compare_artifacts([tcp, uds, raw])

    assert comparison["benchmark_input_gaps"] == []
    assert comparison["transports"]["tcp_ws"]["audio"]["duration_ms"] == 1000.0
    assert comparison["transports"]["uds_ws"]["audio"]["duration_ms"] == 1000.0
    assert comparison["transports"]["raw_uds"]["audio"]["duration_ms"] == 1000.0


def test_compare_artifacts_accepts_duration_milliseconds_benchmark_aliases(tmp_path: Path) -> None:
    tcp = write_artifact(tmp_path / "tcp.json", "tcp_ws", 18.0)
    uds = write_artifact(tmp_path / "uds.json", "uds_ws", 18.0)
    raw = write_artifact(tmp_path / "raw.json", "raw_uds", 13.0)

    for path, duration_key in (
        (tcp, "audio_duration_ms"),
        (uds, "clip_duration_ms"),
        (raw, "source_duration_ms"),
    ):
        payload = json.loads(path.read_text(encoding="utf8"))
        payload["audio"].pop("duration_ms")
        payload["audio"][duration_key] = 1000
        path.write_text(json.dumps(payload), encoding="utf8")

    comparison = compare_module.compare_artifacts([tcp, uds, raw])

    assert comparison["benchmark_input_gaps"] == []
    assert comparison["transports"]["tcp_ws"]["audio"]["duration_ms"] == 1000
    assert comparison["transports"]["uds_ws"]["audio"]["duration_ms"] == 1000
    assert comparison["transports"]["raw_uds"]["audio"]["duration_ms"] == 1000
...
def test_compare_artifacts_blocks_diagnostic_protocol_error_codes(tmp_path: Path) -> None:
    ...
    assert comparison["all_present_transports_protocol_error_free"] is False
    assert comparison["blocking_gaps"] == [
        "raw_uds diagnostic protocol_error_total must be zero; "
