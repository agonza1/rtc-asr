...
def test_manifest_normalizes_common_transport_aliases() -> None:
    ...
    assert normalized_contract_transport("tcp secure websocket transport") == "tcp_ws"
    ...
    assert normalized_contract_transport("unix domain socket transport") == "uds_ws"
    assert normalized_contract_transport("unix-domain-websocket") == "uds_ws"
    assert normalized_contract_transport("unix domain socket websocket transport") == "uds_ws"
    assert normalized_contract_transport("unix domain socket web socket") == "uds_ws"
    assert normalized_contract_transport("websocket uds") == "uds_ws"
    assert normalized_contract_transport("websocket unix socket") == "uds_ws"
    assert normalized_contract_transport("websocket unix domain socket transport") == "uds_ws"
    assert normalized_contract_transport("ws unix") == "uds_ws"
    assert normalized_contract_transport("domain socket web socket transport") == "uds_ws"
    ...
    assert normalized_contract_transport("unix domain socket ws transport") == "uds_ws"
    assert normalized_contract_transport("domain socket ws transport") == "uds_ws"
    ...
    assert normalized_contract_transport("unix raw socket") == "raw_uds"
    assert normalized_contract_transport("unix domain raw socket") == "raw_uds"
    assert normalized_contract_transport("domain socket raw") == "raw_uds"
...
def test_manifest_preserves_nested_cpu_metadata_aliases() -> None:
    ...
    usage_system = extract_system_signals({"metrics": {"cpu": {"usage_percent": 29.75}}})
    ...
    assert usage_system["cpu_utilization_percent"] == 29.75
...
def test_manifest_preserves_nested_power_and_thermal_metadata() -> None:
    ...
    common_alias_system = extract_system_signals(
        {
            "power": {"average_package_watts": 6.2, "energy_j_per_audio_second": 1.8},
            "thermal": {"peak_temp_c": 58.0},
        }
    )
    mean_power_system = extract_system_signals({"metrics": {"power": {"package_power_mean_watts": 5.9}}})
    ...
    assert mean_power_system["package_power_watts"] == 5.9
...
def test_prerender_preserves_common_cpu_power_and_thermal_aliases() -> None:
    system = extract_prerender_system_signals(
        {
            "metrics": {
                "cpu": {"usage_percent": 24.5},
                "power": {"package_power_mean_watts": 6.4, "energy_j_per_audio_second": 1.7},
                "thermal": {"peak_temp_c": 57.5},
            }
        }
    )

    assert system["cpu_utilization_percent"] == 24.5
    assert system["package_power_watts"] == 6.4
    assert system["energy_per_audio_second_j"] == 1.7
    assert system["thermal_peak_celsius"] == 57.5
...
def test_render_llms_points_agents_to_manifest_and_detail_pages() -> None:
    manifest = {
        ...
        "artifacts": [
            ...
            {
                "label": "Demo ASR legacy",
                "slug": "demo-asr",
                "status": "legacy",
                "artifact_path": "benchmark-results/demo-2026-06-13.json",
                "measured_at": "2026-06-13T00:00:00Z",
                "streaming": {"live_metrics_comparable": False},
...
def test_render_homepage_handles_missing_rest_latency() -> None:
    homepage = """<!-- BEGIN GENERATED:static-summary -->\nold\n<!-- END GENERATED:static-summary -->\n<!-- BEGIN GENERATED:generated-at -->\nold\n<!-- END GENERATED:generated-at -->"""
    manifest = {
        "summary": {"validated_count": 1, "tracked_count": 1},
        "tracks": [
            {
                "slug": "validated",
                "label": "Validated Lane",
...
def test_render_homepage_surfaces_raw_uds_transport_coverage() -> None:
    ...
    assert "1 raw UDS artifact" in html
    assert "3 Local STT artifacts are comparable today" in html
    assert "1 artifact catalogs the raw UDS opt-in contract" in html
    assert "1 raw UDS artifacts" not in html


def test_render_homepage_pluralizes_empty_raw_uds_transport_coverage() -> None:
    homepage = """<!-- BEGIN GENERATED:static-summary -->\nold\n<!-- END GENERATED:static-summary -->\n<!-- BEGIN GENERATED:generated-at -->\nold\n<!-- END GENERATED:generated-at -->"""
    manifest = {
        "summary": {
            "validated_count": 1,
            "tracked_count": 1,
            "transport_coverage": {
                "comparable_local_stt_artifact_count": 1,
                "raw_uds_artifact_count": 0,
                "raw_uds_catalog_artifact_count": 2,
            },
        },
        "tracks": [
            {
    ...
    assert "0 raw UDS artifacts" in html
    assert "1 Local STT artifact is comparable today" in html
    assert "2 artifacts catalog the raw UDS opt-in contract" in html
    assert "1 Local STT artifacts are comparable today" not in html
    assert "2 artifact catalogs the raw UDS opt-in contract" not in html
...
def test_make_compose_config_check_covers_supported_compose_profiles() -> None:
    ...
    assert "FASTER_WHISPER_COMPOSE_IMAGE ?= realtime-asr:faster-whisper-cpu" in makefile_text
    assert "QWEN_COMPOSE_IMAGE ?= realtime-asr:qwen-cpu" in makefile_text
    assert "PARAKEET_COMPOSE_IMAGE ?= realtime-asr:parakeet-transformers-cpu" in makefile_text
...
def test_manifest_reads_diagnostic_warning_totals_and_code_counts(tmp_path: Path) -> None:
    artifact_path = tmp_path / "diagnostic-warnings-2026-06-20.json"
    artifact_path.write_text(
        json.dumps(
            {
                "backend": {"name": "demo", "model": "warn-diagnostics-v1", "device": "cpu", "compute_type": "int8"},
                "rest": {"mean_ms": 100, "p95_ms": 140, "rtf_mean": 0.4},
                "streaming": {"partial_mean_ms": 50, "partial_p95_ms": 80, "final_mean_ms": 120, "final_p95_ms": 180},
                "environment": {"date_utc": "2026-06-20T00:00:00Z"},
        ...
        json.dumps(
            {
                "tracks": [
                    {
                        "slug": "diagnostic-warnings",
                        "label": "diagnostic warnings",
                        "backend": "demo",
                        "model": "warn-diagnostics-v1",
                        "device": "cpu",
                        "compute": "int8",
                        "lane": "local",
                        "status": "validated",
                        "status_detail": "diagnostic warning coverage",
                        "target_sample_count": 4,
                        "run_command": "make benchmark",
                    }
                ]
    ...
    assert manifest["tracks"][0]["warnings"] == {
        "received_total": 3,
        "sample_count": 4,
        "rate_per_sample": 0.75,
        "codes": ["partial_dropped", "queue_depth_high"],
    }
    assert manifest["summary"]["warnings"]["rate_per_sample"] == 0.75
