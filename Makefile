...
LOW_LATENCY_SWEEP_CHUNK_MS ?= 80 100 200 250
...
mlx-venv:
	...
	@if [ -x $(MLX_PYTHON) ] && $(MLX_PYTHON) -c "import sys" >/dev/null 2>&1; then \
		...
		$(MLX_PYTHON) -m pip install --upgrade pip parakeet-mlx psutil; \
	fi
...
benchmark-faster-whisper-small-low-latency-sweep: venv
	...
	@{ set -e; 	for chunk in $(LOW_LATENCY_SWEEP_CHUNK_MS); do 		for window in $(LOW_LATENCY_SWEEP_PARTIAL_WINDOWS); do 			for frame in $(LOW_LATENCY_SWEEP_BINARY_FRAMES); do 				if [ "$$frame" = "true" ]; then frame_flag="--binary-frames"; frame_label="binary"; else frame_flag=""; frame_label="json"; fi; 				window_slug=$$(printf '%s' "$$window" | tr '.' '_'); 				output="$(BENCHMARK_RESULTS_DIR)/faster-whisper-$(FASTER_WHISPER_SMALL_MODEL)-$(FASTER_WHISPER_COMPUTE_TYPE)-c$${chunk}-w$${window_slug}-$${frame_label}-$(BENCHMARK_RESULT_DATE).json"; 				echo "  -> $$output"; 				$(PYTHON) tests/benchmark.py --spawn-server --model $(FASTER_WHISPER_SMALL_MODEL) --compute-type $(FASTER_WHISPER_COMPUTE_TYPE) --sample-count $(LOW_LATENCY_SWEEP_SAMPLE_COUNT) --rest-runs $(LOW_LATENCY_SWEEP_REST_RUNS) --chunk-ms $$chunk --partial-window $$window $$frame_flag --request-retries $(BENCHMARK_REQUEST_RETRIES) --request-retry-delay $(BENCHMARK_REQUEST_RETRY_DELAY) --output "$$output"; 			done; 		done; 	done; 	}
...
benchmark-qwen-mps-low-latency-sweep: venv
	...
	@mkdir -p .cache/huggingface
	@{ set -e; 	for chunk in $(LOW_LATENCY_SWEEP_CHUNK_MS); do 		for window in $(LOW_LATENCY_SWEEP_PARTIAL_WINDOWS); do 			for frame in $(LOW_LATENCY_SWEEP_BINARY_FRAMES); do 				if [ "$$frame" = "true" ]; then frame_flag="--binary-frames"; frame_label="binary"; else frame_flag=""; frame_label="json"; fi; 				window_slug=$$(printf '%s' "$$window" | tr '.' '_'); 				output="$(BENCHMARK_RESULTS_DIR)/qwen-mps-c$${chunk}-w$${window_slug}-$${frame_label}-$(BENCHMARK_RESULT_DATE).json"; 				echo "  -> $$output"; 				ASR_BACKEND=qwen-asr ASR_DEVICE=mps ASR_QWEN_DEVICE_MAP= ASR_QWEN_MODEL=$(QWEN_MPS_MODEL) ASR_QWEN_DTYPE=$(QWEN_MPS_DTYPE) $(PYTHON) tests/benchmark.py --spawn-server --backend qwen-asr --model $(QWEN_MPS_MODEL) --device mps --qwen-dtype $(QWEN_MPS_DTYPE) --sample-count $(LOW_LATENCY_SWEEP_SAMPLE_COUNT) --rest-runs $(LOW_LATENCY_SWEEP_REST_RUNS) --chunk-ms $$chunk --partial-interval-chunks $(BENCHMARK_PARTIAL_INTERVAL_CHUNKS) --partial-window $$window $$frame_flag --request-retries $(BENCHMARK_REQUEST_RETRIES) --request-retry-delay $(BENCHMARK_REQUEST_RETRY_DELAY) --output "$$output"; 			done; 		done; 	done; 	}
