...
def render_row(
    ...
    return "".join(
        [
            ...
            f'<td data-label="Partial cadence"><strong>{format_ms(partial_value)}</strong><div class="tiny">P95 {format_ms(partial_backlog_p95(entry))}</div><div class="tiny">{delta_text(partial_delta)} vs tightest cadence</div></td>',
...
<div class="comparison-wrap panel" id="comparison-wrap">
  <div class="comparison-scroll">
    <table>
      <thead>
        <tr><th>Lane</th><th>State</th><th>Score</th><th>{hint(ttfb_first_partial_label(), ttfb_first_partial_description())}</th><th>{hint('Partial cadence', 'Time gap between visible partial transcript updates after streaming is underway; read it alongside late partial ratio and transcript churn.')}</th><th>{hint('Audio-end finalization', 'Time from audio end until the final transcript returns; this is closeout delay, not total clip duration.')}</th><th>{hint('REST throughput context', 'Batch request latency for the same backend outside the streaming websocket path. Keep this as throughput context rather than the main live turn-taking signal.')}</th><th>{hint('Samples', 'How many benchmark samples were recorded for this published artifact.')}</th><th>Details</th></tr>
      </thead>
      <tbody>
{rows}
      </tbody>
