...
The demo sidecar exposes `GET /rtc-asr/ready` with `status`, `ready`, `bridge_status`, and `can_start_session` fields so compose and local probes can distinguish a usable Pipecat bridge from a degraded optional-runtime install.
