...
The demo sidecar exposes `GET /rtc-asr/ready` with `status`, `ready`, `bridge_status`, `can_start_session`, `rtc_asr_ws_url`, and selected ASR target fields so compose and local probes can distinguish a usable Pipecat bridge from a degraded optional-runtime install.
